"""Thin CLI entry point for Phase 4 transformation.
...
same service can later be wrapped as a Dagster asset (Phase 5) without change.
"""
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Transform Landing Zone documents (Phase 4).")
...
def main(argv: list[str] | None = None) -> int:
    ...
    except Exception as exc:  # noqa: BLE001 -- a run-level failure (Mongo/MinIO unreachable,
        # auth/config error, etc.) must be surfaced clearly as a structured event and a
        # distinct exit code, not a bare traceback -- individual document failures never
        # reach here, they're already caught and counted inside TransformService.
        logger.error(json.dumps({"event": "run_failed", "reason": repr(exc)}))
        return 3
    finally:
